...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        successors = []
        currentPosition, visitedCorners = state # Correctly unpack the state [2]
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = currentPosition
            dx, dy = Actions.directionToVector(action) # Use singular 'directionToVector' [4, 5]
            nextx, nexty = int(x + dx), int(y + dy) # Use addition, not a comma [4]
                
            if not self.walls[nextx][nexty]:
                nextPos = (nextx, nexty)
                newVisitedList = list(visitedCorners)
                for i in range(len(self.corners)):
                    if nextPos == self.corners[i]:
                        newVisitedList[i] = True
                    
                nextState = (nextPos, tuple(newVisitedList))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    distances = [abs(pos[0] - c[0]) + abs(pos[1] - c[1]) for c in unvisited]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodList:
        key = ('mazeDistance' , position , food)
        if key not in cache:
